...
#
#  rewrited by zhe.chen <chenzhe07@gmail.com> 20131011
#  1. Add slave check and relay_log_purge check.
#  2. Table use Memory engines output.
#  3. Fragmente info output more detailed.
#  4. More Innodb parameter info typeout.
use strict;
use warnings;
use diagnostics;
use File::Spec;
...
sub get_replication_status {
        ...
        if ($io_running eq 'Yes' && $sql_running eq 'Yes') {
            ...
            badprint "Slave error: Io_running:$io_running, Sql_running:$sql_running.\n";
        }

        if ($myvar{'relay_log_purge'} eq 'ON') {
            goodprint "Slave relay_log_purge On.\n";
        } else {
            badprint "Slave relay_log_purge Off.\n"; 
        }
...
sub security_recommendations {
    ...
    if (@table_mem) {
         foreach my $info (sort @table_mem) {
            chomp($info);
            badprint "$info use Memory engine.\n";
         }
    } else {
         goodprint "No table use Memory engine.\n";
    }

    # expire_logs_days info.
    if ($myvar{'expire_logs_days'} == 0) {
        badprint "Expire logs days : $myvar{expire_logs_days}\n";
    } else {
...
sub check_storage_engines {
	...
	$engines .= (defined $myvar{'have_myisam'} && $myvar{'have_myisam'} eq "YES")? greenwrap "+MyIsam " : redwrap "-MyIsam " ;
	$engines .= (defined $myvar{'have_memory'} && $myvar{'have_memory'} eq "YES")? greenwrap "+Memory " : redwrap "-Memory " ;
	$engines .= (defined $myvar{'have_csv'} && $myvar{'have_csv'} eq "YES")? greenwrap "+Csv " : redwrap "-Csv " ;
	...
	if (!defined $enginestats{'MyISAM'} && defined $myvar{'have_myisam'} && $myvar{'have_myisam'} eq "YES") {
...
sub calculations {
	...
	$mycalc{'server_buffers'} += (defined $myvar{'join_buffer_size'}) ? $myvar{'join_buffer_size'} : 0 ;
	$mycalc{'server_buffers'} += (defined $myvar{'myisam_sort_buffer_size'}) ? $myvar{'myisam_sort_buffer_size'} : 0 ;
	$mycalc{'server_buffers'} += (defined $myvar{'preload_buffer_size'}) ? $myvar{'preload_buffer_size'} : 0 ;
	...
	if ($myvar{'innodb_adaptive_hash_index'} eq "ON") {
		$mycalc{'innodb_ad_hash_info'} = "hash_cells: $mystat{'Innodb_adaptive_hash_cells'}, heap_buffer: $mystat{'Innodb_adaptive_hash_heap_buffers'}, hash_search: $mystat{'Innodb_adaptive_hash_hash_searches'}, non_hash_serach: $mystat{'Innodb_adaptive_hash_non_hash_searches'}";
	}
...
sub mysql_stats {
	...
	if (!defined($mycalc{'total_myisam_indexes'}) and $doremote == 1) {
		...
		if ($mystat{'Key_read_requests'} > 0) {
            ...
            print "     +---- Key_read_requests: No queries have run that would use keys\n";
		}
	}
	...
	if (defined $myvar{'have_innodb'} && $myvar{'have_innodb'} eq "YES" && defined $enginestats{'InnoDB'}) {
        ...
        infoprint "Adaptive hash index info  : $mycalc{'innodb_ad_hash_info'}\n";
        infoprint "Innodb block wait timeout  : $myvar{'innodb_lock_wait_timeout'}\n";
        infoprint "Innodb thread concurrency : $myvar{'innodb_thread_concurrency'}\n";

        if ($myvar{'innodb_fast_shutdown'} == 1) {
            goodprint "Innodb fast shutdown : $myvar{'innodb_fast_shutdown'}\n";
        } else {
            badprint "Innodb fast shutdown : $myvar{'innodb_fast_shutdown'}\n";
        }
...
# BEGIN 'MAIN'
# ---------------------------------------------------------------------------
print	"\n >>  MySQLTuner $tunerversion - rewrited by zhe.chen <chenzhe07\@gmail.com>\n".
